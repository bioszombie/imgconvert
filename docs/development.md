...
- `requirements-build.txt` — runtime dependency plus pinned PyInstaller used to make standalone executables
- `requirements-dev.txt` — runtime dependency plus pinned test/security/lint tools
- `pyproject.toml` — package metadata and pinned build backend
...
Then build and verify a platform-local artifact:
...
A local build is useful for debugging but is not a substitute for the four-platform GitHub Actions package matrix used for releases.
...
4. open a PR
5. require source CI and Package CI to pass
6. merge to `master`
7. create the exactly matching `vMAJOR.MINOR.PATCH` tag
8. let the tag-triggered Package workflow create the release

The workflow rejects mismatched or non-stable version tags.

## Release scripts

Files under `scripts/` participate in CI linting, compilation, and Bandit SAST because they execute inside the trusted release pipeline. Treat changes to packaging/checksum/tag-validation scripts as supply-chain changes, not convenience scripting.
